...
__global__ void refine_inv_kernel(
    ...
    b_xi[idx - n0] = (vec[k] - dot(mat + tri(k, 0), vec, k)) / mat[tri(k, k)]; // x = (v - mean) / std
}

template <size_t MAX_K, size_t N_DIM, typename i_t, typename f_t>
__host__ void refine_inv(
    cudaStream_t stream,
    const f_t* points,
    const i_t* neighbors,
    const i_t* offsets,
    ...
    // copy values to initial values
    batch_copy<<<cld(n_batches * n0, 256), 256, 0, stream>>>(values, initial_values, n_batches, n_points, n0, n0);

    // walk backwards through levels and compute inverse
    for (size_t level = n_levels; level-- > 1;) {
        size_t start_idx = offsets_host[level - 1];
        size_t end_idx = offsets_host[level];
        size_t n_threads = (end_idx - start_idx) * n_batches;
        refine_inv_kernel<MAX_K, N_DIM, i_t, f_t><<<cld(n_threads, 256), 256, 0, stream>>>(
            points,
            neighbors,
            cov_bins,
            cov_vals,
